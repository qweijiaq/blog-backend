...
type ArticleUpdateRequest struct {
	...
	BannerID int      `json:"banner_id"` // 文章封面id
...
func (ArticleApi) ArticleUpdateView(c *gin.Context) {
	...
	err = es_server.ArticleUpdate(cr.ID, DataMap)
	if err != nil {
	...
	// 更新成功，同步数据到全文搜索
	newArticle, _ := es_server.CommDetail(cr.ID)
	if article.Content != newArticle.Content || article.Title != newArticle.Title {
		es_server.DeleteFullTextByArticleId(cr.ID)
		es_server.AsyncArticleByFullText(cr.ID, article.Title, newArticle.Content)
	}
