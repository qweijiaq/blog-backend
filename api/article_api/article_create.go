...
type ArticleRequest struct {
	...
	Category ctype.Array `json:"category"`                                // 文章分类
...
func (ArticleApi) ArticleCreateView(c *gin.Context) {
	...
	if cr.BannerID == 0 {
		...
		cr.BannerID = int(bannerIDList[rand.Intn(len(bannerIDList))])
	}
	...
	go es_server.AsyncArticleByFullText(article.ID, article.Title, article.Content)
